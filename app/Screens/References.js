...
export default function References() {
        ...
        [
          { text: "Cancel", style: "cancel" },
          {
            ...
            onPress: async () => {
              ...
              if (index !== -1) {
                const updated = drafts[index].references.filter(
                  (_, i) => !selectedIndices.includes(i)
                );
                drafts[index].references = updated;
  ...
  const handleAddReference = () => {
    navigation.navigate("AddReference", { draftId });
  };

  const handleEditReference = (index) => {
    if (!deleting) {
      navigation.navigate("AddReference", { draftId, index });
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (index !== -1) {
      drafts[index].references = references;
      await AsyncStorage.setItem("resume_drafts", JSON.stringify(drafts));
      ...
      const currentIndex = enabledSections.indexOf("references");
  ...
  useFocusEffect(
    React.useCallback(() => {
      const onBackPress = async () => {
        ...
        const currentIndex = enabled.indexOf("references"); 
        ...
        return (
          <View key={index} style={styles.row}>
            ...
            <TouchableOpacity
              ...
              onPress={() => handleEditReference(index)}
            >
              <Text style={styles.buttonText}>{ref.name}</Text>
            </TouchableOpacity>
          </View>
        );
      })}
