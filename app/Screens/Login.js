      ...
      try {
        ...
        if (token) {
          const response = await axios.get(
            "http://192.168.2.206:3000/api/auth/verify", // do make sure to include your IP address here
            {
              headers: {
  ...
  const handleLogin = async (values) => {
    ...
    try {
      const response = await axios.post(
        "http://192.168.2.206:3000/api/auth/login", // do make sure to include your IP address here
        {
          email: values.email,
