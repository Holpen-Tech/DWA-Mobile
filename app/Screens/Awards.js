...
export default function Awards() {
  ...
  const handleAddAward = () => {
    navigation.navigate("AddAward", { draftId });
  };

  const handleEditAward = (index) => {
    if (!deleting) {
      navigation.navigate("AddAward", { draftId, index });
    }
  };
        ...
        [
          { text: "Cancel", style: "cancel" },
          {
            ...
            onPress: async () => {
              ...
              if (index !== -1) {
                const updated = drafts[index].awards.filter((_, i) => !selectedIndices.includes(i));
                drafts[index].awards = updated;
  ...
  const handleSave = async () => {
    ...
    if (index !== -1) {
      ...
      const currentIndex = enabledSections.indexOf("awards");
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <TouchableOpacity style={styles.addButton} onPress={handleAddAward}>
        <Text style={styles.buttonText}>+ Add Award</Text>
      </TouchableOpacity>
